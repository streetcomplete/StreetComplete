...
| MapLibre Compose snapshot | Clean `--refresh-dependencies` compilation plus dependency insight for desktop, Android runtime, and iOS simulator compile configurations | Pass | Core, location, target variants, iOS resources, Android OpenGL, macOS location, and desktop Metal runtime resolve from the `0.15.1-SNAPSHOT` series. The exact validated publication is recorded in `03-maplibre-compose-upstream.md`. |
| Cross-host desktop runtime resolution | Run `dependencyInsight` with simulated Linux x64 and Windows ARM64 JVM host properties | Pass | Linux resolves the Vulkan x64 and `location-runtime-linux` artifacts; Windows resolves the Vulkan ARM64 and `location-runtime-windows` artifacts from the snapshot repository. |
...
| Production target matrix | `./gradlew :app:linkDebugFrameworkIosSimulatorArm64 :androidApp:assembleDebug :app:createDistributable` | Pass | The validated snapshot stack produces the iOS framework, Android APK, and native desktop application together after lifecycle and host-ownership fixes. |
...
| Cross-target production compilation | `./gradlew :androidApp:assembleDebug :app:compileKotlinDesktop :app:compileKotlinIosSimulatorArm64 :app:desktopTest --tests '*PinsLayersTest' --tests '*StyledElementTest' --tests '*MainMapContentStateTest' --continue` | Pass | The stable-source/current-handle update path and its tests compile on Android, desktop, and Kotlin/Native against the exact snapshot. |
