...
@Serializable sealed interface StreetParking {
    ...
    @Serializable data object None : StreetParking

    /** When an unknown/unsupported value has been used */
    @Serializable data object Unknown : StreetParking

    /** When not both parking orientation and position have been specified*/
    @Serializable data object Incomplete : StreetParking

    /** There is street parking, but it is mapped as separate geometry */
    @Serializable data object Separate : StreetParking

    @Serializable data class PositionAndOrientation(
...
private val StreetParking.isValid: Boolean get() = when (this) {
    StreetParking.Incomplete, StreetParking.Unknown -> false
    else -> true
}
...
val StreetParking.estimatedWidthOnRoad: Float get() = when (this) {
    is StreetParking.PositionAndOrientation -> orientation.estimatedWidth * position.estimatedWidthOnRoadFactor
    else -> 0f // otherwise let's assume it's not on the street itself
}

val StreetParking.estimatedWidthOffRoad: Float get() = when (this) {
    is StreetParking.PositionAndOrientation -> orientation.estimatedWidth * (1 - position.estimatedWidthOnRoadFactor)
    else -> 0f // otherwise let's assume it's not on the street itself
}
