 ...
 *    right
 */
@Composable fun LanesSelectPuzzle(
    ...
    hasCenterLeftTurnLane: Boolean = false,
    ...
    Box(modifier = modifier) {
        ...
        Canvas(Modifier.fillMaxSize()) {
            ...
            // draw background
            val backgroundLeft = if (laneCountLeft != null) 0f else laneWidth * leftLanesEnd
            val backgroundRight = if (laneCountRight != null) 0f else size.width - laneWidth * rightLanesStart
            drawRect(
        ...
        if (laneCountLeft != 0) {
            Spacer(
                Modifier
                    ...
                    .clickable {
                        if (isLeftHandTraffic) onClickForwardSide() else onClickBackwardSide()
                    }
            )
        }
        ...
        if (laneCountRight != 0) {
            Spacer(
                Modifier
                    ...
                    .clickable {
                        if (isLeftHandTraffic) onClickBackwardSide() else onClickForwardSide()
                    }
            )
        }
    }
...
private fun LanesPainterPreview() {
    var lanesForward by remember { mutableIntStateOf(1) }
    var lanesBackward by remember { mutableIntStateOf(1) }
    var isLeftHandTraffic by remember { mutableStateOf(false) }
    var hasCenterLeftTurnLane by remember { mutableStateOf(false) }
    var hasMarkings by remember { mutableStateOf(true) }

    Column {
        LanesSelectPuzzle(
            laneCountForward = lanesForward.takeIf { it >= 0 },
            laneCountBackward = lanesBackward.takeIf { it >= 0 },
            onClickForwardSide = {},
            onClickBackwardSide = {},
            isLeftHandTraffic = isLeftHandTraffic,
            hasCenterLeftTurnLane = hasCenterLeftTurnLane,
            isShowingLaneMarkings = hasMarkings,
            modifier = Modifier.size(360.dp)
        )
        Column {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("Lanes left/right")
                StepperButton(onIncrease = { lanesForward++ }, onDecrease = { lanesForward-- })
                StepperButton(onIncrease = { lanesBackward++ }, onDecrease = { lanesBackward-- })
            }
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("Left hand traffic")
                Checkbox(isLeftHandTraffic, onCheckedChange = { isLeftHandTraffic = it })
            }
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("Center left turn lane")
                Checkbox(hasCenterLeftTurnLane, onCheckedChange = { hasCenterLeftTurnLane = it })
            }
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("Road markings")
                Checkbox(hasMarkings, onCheckedChange = { hasMarkings = it })
            }
        }
    }
}
