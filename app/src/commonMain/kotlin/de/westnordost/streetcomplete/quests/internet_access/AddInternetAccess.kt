...
class AddInternetAccess : OsmFilterQuestType<Set<InternetAccess>>() {
    ...
    override val icon = Res.drawable.quest_wifi
    override val title = Res.string.quest_internet_access_title
    override val achievements = listOf(CITIZEN)
    override val defaultDisabledMessage = Res.string.default_disabled_msg_go_inside

    @Composable
    override fun Form(on: (QuestAction<Set<InternetAccess>>) -> Unit, element: Element, geometry: ElementGeometry, countryInfo: CountryInfo) {
        AddInternetAccessForm(on)
    }
