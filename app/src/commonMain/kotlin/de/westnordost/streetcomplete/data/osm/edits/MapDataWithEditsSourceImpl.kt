...
class MapDataWithEditsSourceImpl(
    private val mapDataSource: MapDataSource,
    private val elementEditsSource: ElementEditsSource,
    private val elementGeometryCreator: ElementGeometryCreator
) : MapDataWithEditsSource {

    private val listeners = Listeners<MapDataWithEditsSource.Listener>()
        ...
        override fun onDeletedEdits(edits: List<ElementEdit>) {
            ...
            lock.withLock {
                ...
                deletedElementKeys = edits
                    .flatMap { elementEditsSource.getIdProvider(it.id).getAll() }
                    .toMutableList()
    ...
    override fun get(type: ElementType, id: Long): Element? = lock.withLock {
        ...
        return updatedElements[key] ?: mapDataSource.get(type, id)
    }

    override fun getGeometry(type: ElementType, id: Long): ElementGeometry? = lock.withLock {
        val key = ElementKey(type, id)
        if (deletedElements.contains(key)) return null
        ...
        return if (updatedGeometries.containsKey(key)) {
            ...
            mapDataSource.getGeometry(type, id)
        }
    }

    override fun getGeometries(keys: Collection<ElementKey>): List<ElementGeometryEntry> = lock.withLock {
        ...
        val originalGeometries = mapDataSource.getGeometries(originalKeys)
        return updatedGeometries + originalGeometries
    }

    override fun getMapDataWithGeometry(bbox: BoundingBox): MapDataWithGeometry = lock.withLock {
        val mapDataWithGeometry = mapDataSource.getMapDataWithGeometry(bbox)
        modifyBBoxMapData(bbox, mapDataWithGeometry)
    ...
    private fun getNodes(ids: Set<Long>): Collection<Node> = lock.withLock {
        val nodes = mapDataSource.getNodes(ids)
    ...
    override fun getWaysForNode(id: Long): Collection<Way> = lock.withLock {
        ...
        mapDataSource.getWaysForNode(id).associateByTo(waysById) { it.id }
    ...
    override fun getRelationsForNode(id: Long): Collection<Relation> =
        _getRelationsForElement(NODE, id)

    override fun getRelationsForWay(id: Long): Collection<Relation> =
        _getRelationsForElement(WAY, id)

    override fun getRelationsForRelation(id: Long): Collection<Relation> =
        _getRelationsForElement(RELATION, id)

    private fun _getRelationsForElement(type: ElementType, id: Long): Collection<Relation> = lock.withLock {
        val relationsById = HashMap<Long, Relation>()
        val relations = mapDataSource.getRelationsForElement(type, id)
    ...
    private fun rebuildLocalChanges() = lock.withLock {
        ...
        updatedElements.clear()
        updatedGeometries.clear()
        val edits = elementEditsSource.getAllUnsynced()
        for (edit in edits) {
            applyEdit(edit)
        }
    }
    ...
    private fun applyEdit(edit: ElementEdit): MapDataUpdates? = lock.withLock {
        val idProvider = elementEditsSource.getIdProvider(edit.id)
    ...
    override fun addListener(listener: MapDataWithEditsSource.Listener) {
        listeners.add(listener)
    }
    override fun removeListener(listener: MapDataWithEditsSource.Listener) {
