...
fun FocusedGeometryLayers(geometry: ElementGeometry?) {
    ...
    val cycleFraction = if (geometry != null) {
        key(geometry) {
            val transition = rememberInfiniteTransition(label = "Focused geometry")
            val fraction by transition.animateFloat(
                initialValue = 0f,
                targetValue = 1f,
                animationSpec = infiniteRepeatable(tween(HIGHLIGHT_CYCLE_MILLIS, easing = LinearEasing)),
    ...
    // Compose owns the animation timing; native transitions must not interpolate its frames again.
    val noTransition = remember { TransitionOptions(duration = Duration.ZERO) }
    ...
    LineLayer(
        ...
        opacityTransition = noTransition,
        color = const(HighlightColor),
        width = const(style.lineWidth.dp),
        widthTransition = noTransition,
...
private fun focusedGeometryData(geometry: ElementGeometry): GeoJsonData =
