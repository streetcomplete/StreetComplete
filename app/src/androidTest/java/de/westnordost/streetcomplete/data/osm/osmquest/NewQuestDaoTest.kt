...
class NewQuestDaoTest : ApplicationDbTestCase() {
    private lateinit var geometryDao: ElementGeometryDao
    private lateinit var dao: NewOsmQuestDao
    ...
    @Before fun createDao() {
        ...
        dao = NewOsmQuestDao(dbHelper, NewOsmQuestMapping(QuestTypeRegistry(list), elementGeometryEntryMapping.geometryMapping))
    }
    ...
    @Test fun getAllInBBox() {
        ...
        addToDaos(TEST_QUEST_TYPE, Element.Type.NODE, 4L, p(0.5,1.5))
        addToDaos(TEST_QUEST_TYPE, Element.Type.NODE, 4L, p(1.5,0.5))
        assertTrue(dao.getAllInBBox(BoundingBox(0.0,0.0,1.0,1.0)).containsExactlyInAnyOrder(listOf(q1,q2)))
    }
    ...
    private fun addToDaos(
        ...
        val q = NewOsmQuest(null, questType, elementType, elementId, g)
        dao.add(q)
        return q
    }
