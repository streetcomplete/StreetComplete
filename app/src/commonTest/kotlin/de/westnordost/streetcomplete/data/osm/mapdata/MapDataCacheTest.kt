...
internal class MapDataCacheTest {
    ...
    @Test fun `update puts way geometry`() {
        ...
        val geometryDB = mock<ElementGeometryDao>()
    ...
    @Test fun `getElement also caches node if not in spatialCache`() {
        ...
        val cache = getEmptyMapDataCache()
        val elementDB = mock<ElementDao>()
        every { elementDB.get(ElementType.NODE, 1L) } sequentially {
            returns(node)
            repeat { returns(null) }
        }
        ...
        val elementDB2 = mock<ElementDao>()
        every { elementDB2.get(ElementType.NODE, 1L) } sequentially {
            returns(node)
            repeat { returns(null) }
        }
        assertEquals(node, cache.getElement(ElementType.NODE, 1L) { type, id -> elementDB2.get(type, id) })
        verifyNoMoreCalls(elementDB2)
    }
    ...
    @Test fun `getElement fetches and caches way`() {
        ...
        val cache = getEmptyMapDataCache()
        val elementDB = mock<ElementDao>()
        every { elementDB.get(ElementType.WAY, 2L) } sequentially {
            returns(way)
    ...
    @Test fun `getElement fetches and caches relation`() {
        ...
        val cache = getEmptyMapDataCache()
        val elementDB = mock<ElementDao>()
        every { elementDB.get(ElementType.RELATION, 1L) } sequentially {
            returns(rel)
            throws(IllegalStateException())
        }
    ...
    @Test fun `getGeometry fetches node geometry if not in spatialCache`() {
        ...
        val cache = getEmptyMapDataCache()
        val geometryDB = mock<ElementGeometryDao>()
        every { geometryDB.get(ElementType.NODE, 2L) } sequentially {
            returns(geo)
            throws(IllegalStateException())
        }
        ...
        val geometryDB2 = mock<ElementGeometryDao>()
        every { geometryDB2.get(ElementType.NODE, 2L) } sequentially {
            returns(geo)
            throws(IllegalStateException())
        }
    ...
    @Test fun `getGeometry fetches and caches way geometry`() {
        ...
        val cache = getEmptyMapDataCache()
        val geometryDB = mock<ElementGeometryDao>()
        every { geometryDB.get(ElementType.WAY, 2L) } sequentially {
            returns(geo)
            throws(IllegalStateException())
        }
    ...
    @Test fun `getGeometry fetches and caches relation geometry`() {
        ...
        val cache = getEmptyMapDataCache()
        val geometryDB = mock<ElementGeometryDao>()
        every { geometryDB.get(ElementType.RELATION, 2L) } sequentially {
            returns(geo)
            throws(IllegalStateException())
        }
        // get way 2 and verify the fetch function is called, but only once
    ...
    @Test fun `getNodes doesn't fetch cached nodes`() {
        ...
        assertTrue(cache.getNodes(nodes.map { it.id }).containsExactlyInAnyOrder(nodes))
    }
    ...
    @Test fun `getNodes fetches only nodes not in cache`() {
        ...
        val nodeDB = mock<NodeDao>()
        every { nodeDB.getAll(listOf(uncachedNode.id)) } returns listOf(uncachedNode)
        assertTrue(
            cache.getNodes(allNodes.map { it.id }) { nodeDB.getAll(it) }
                .containsExactlyInAnyOrder(allNodes)
        )
    ...
    @Test fun `getNodes does not fetch cached nodes`() {
        ...
        val nodeDB = mock<NodeDao>()
        assertTrue(
            cache.getNodes(nodes.map { it.id }) { nodeDB.getAll(it) }
                .containsExactlyInAnyOrder(nodes)
        )
        verifyNoMoreCalls(nodeDB)
    }
    ...
    @Test fun `getNodes fetches formerly cached nodes outside spatial cache after trim`() {
        ...
        assertTrue(
            cache.getNodes(nodes.map { it.id }) { nodeDB.getAll(it.sorted()) }
                .containsExactlyInAnyOrder(nodes)
        )
    ...
    @Test fun `getElements doesn't fetch cached elements`() {
        ...
        assertTrue(
            cache.getElements(elements.map { ElementKey(it.type, it.id) })
    ...
    @Test fun `getElements fetches only elements not in cache`() {
        ...
        assertTrue(
            cache.getElements(elements.map { ElementKey(it.type, it.id) }) { elementDB.getAll(it.toHashSet()) }
                .containsExactlyInAnyOrder(elements)
        )
        ...
        assertTrue(
            cache.getElements(elements.map { ElementKey(it.type, it.id) }) { elementDB.getAll(it) }
                .containsExactlyInAnyOrder(elements)
        )
        verifyNoMoreCalls(elementDB)
    }
    ...
    @Test fun `getGeometries doesn't fetch cached geometries`() {
        ...
        assertTrue(
            cache.getGeometries(entries.map { ElementKey(it.elementType, it.elementId) })
    ...
    @Test fun `getWaysForNode caches from db`() {
        ...
        val wayDB = mock<WayDao>()
        every { wayDB.getAllForNode(1L) } sequentially {
            returns(listOf(way1, way2))
            throws(IllegalStateException())
        }
    ...
    @Test fun `getWaysForNode does not return just deleted way`() {
        ...
        assertTrue(cache.getWaysForNode(1L).containsExactlyInAnyOrder(listOf(way1, way2)))
        assertTrue(cache.getWaysForNode(2L).containsExactlyInAnyOrder(listOf(way1)))
        assertTrue(cache.getWaysForNode(3L).containsExactlyInAnyOrder(listOf(way2)))

        cache.update(deletedKeys = listOf(ElementKey(way2.type, way2.id)))
        assertTrue(cache.getWaysForNode(1L).containsExactlyInAnyOrder(listOf(way1)))
        assertTrue(cache.getWaysForNode(2L).containsExactlyInAnyOrder(listOf(way1)))
        assertTrue(cache.getWaysForNode(3L).isEmpty())
    }
    ...
    @Test fun `getWaysForNode returns updated way that now does contain the node but before it didn't`() {
        ...
        assertTrue(cache.getWaysForNode(1L).containsExactlyInAnyOrder(listOf(way1, way2updated)))
        assertTrue(cache.getWaysForNode(2L).containsExactlyInAnyOrder(listOf(way1, way2updated)))
        assertTrue(cache.getWaysForNode(3L).containsExactlyInAnyOrder(listOf(way2updated)))
    }
    ...
    @Test fun `getRelationsForNode and Way does not return just deleted relation`() {
        ...
        val cache = getEmptyMapDataCache()
        cache.update(updatedElements = listOf(node1, node2, way1, rel1, rel2), bbox = nodesRect.asBoundingBox(16))
        assertTrue(cache.getRelationsForNode(1L).containsExactlyInAnyOrder(listOf(rel1, rel2)))
        assertTrue(cache.getRelationsForWay(1L).containsExactlyInAnyOrder(listOf(rel2)))

        cache.update(deletedKeys = listOf(ElementKey(rel2.type, rel2.id)))
        assertTrue(cache.getRelationsForNode(1L).containsExactlyInAnyOrder(listOf(rel1)))
        assertTrue(cache.getRelationsForWay(1L).isEmpty())
    }
    ...
    @Test fun `getRelationsForNode and Way returns updated relation that now does contain the element but before it didn't`() {
        ...
        assertTrue(cache.getRelationsForNode(1L).containsExactlyInAnyOrder(listOf(rel1updated)))
        assertTrue(cache.getRelationsForWay(1L).containsExactlyInAnyOrder(listOf(rel2, rel1updated)))
    }
    ...
    @Test fun `getRelationsForNode and Way doesn't return updated relation that used to contain the element before but now it doesn't`() {
        ...
        cache.update(updatedElements = listOf(node1, node2, way1, rel1, rel2), bbox = nodesRect.asBoundingBox(16))
        assertTrue(cache.getRelationsForNode(1L).containsExactlyInAnyOrder(listOf(rel1, rel2)))
        assertTrue(cache.getRelationsForWay(1L).containsExactlyInAnyOrder(listOf(rel1, rel2)))
        ...
        assertTrue(cache.getRelationsForNode(1L).containsExactlyInAnyOrder(listOf(rel2updated)))
        assertTrue(cache.getRelationsForWay(1L).containsExactlyInAnyOrder(listOf(rel1updated)))
    }
    ...
    @Test fun `cache returns nodes outside the BBox if part of ways`() {
        ...
        val cache = getEmptyMapDataCache()
        cache.update(
            updatedElements = nodes + ways + listOf(outsideNode, fullyOutsideNode, outsideWay, rel1, outsideRel),
            bbox = nodesRect.asBoundingBox(16)
        )
    ...
    @Test fun `add relation with members already in spatial cache`() {
        ...
        assertEquals(listOf(rel1), cache.getRelationsForNode(1))
        assertEquals(listOf(rel2), cache.getRelationsForWay(1))
    }
    ...
    @Test fun `clear clears`() {
        ...
        // not empty
        assertTrue(
            cache.getRelationsForNode(1L) { emptyList() }
                .containsExactlyInAnyOrder(listOf(rel1))
        )
        cache.clear()
        assertEquals(emptyList<Relation>(), cache.getRelationsForNode(1L) { emptyList() })
        assertEquals(
            emptyList<Element>(),
            cache.getElements(listOf(node1, node2, way1, rel1, rel2)
                .map { ElementKey(it.type, it.id) }) { emptyList() }
        )
    }
    ...
    @Test fun `update doesn't create waysByNodeId entry if node is not in spatialCache`() {
        ...
        assertTrue(
            cache.getWaysForNode(1L) { wayDB.getAllForNode(it) }
                .containsExactlyInAnyOrder(listOf(way1))
        )
        verify { wayDB.getAllForNode(1L) } // was fetched from cache

        every { wayDB.getAllForNode(2L) } returns listOf(way1)
        assertTrue(
            cache.getWaysForNode(2L) { wayDB.getAllForNode(it) }
                .containsExactlyInAnyOrder(listOf(way1))
        )
    ...
    @Test fun `update does create waysByNodeId entry if node is in spatialCache`() {
        ...
        assertTrue(
            cache.getWaysForNode(1L)
                .containsExactlyInAnyOrder(listOf(way1))
        )
        assertTrue(
            cache.getWaysForNode(2L)
                .containsExactlyInAnyOrder(listOf(way1))
        )
    }
    ...
    @Test fun `update does add way to waysByNodeId entry if entry already exists`() {
        ...
        assertTrue(
            cache.getWaysForNode(1L)
                .containsExactlyInAnyOrder(listOf(way1, way2))
        )
    ...
    @Test fun `update does create relationsByElementKey entry if element is referenced by spatialCache`() {
        ...
        val node3 = node(3, LatLon(0.0002, 0.0002))
        val nodes = listOf(node1, node2, node3)
        val nodesRect = listOf(node1.position, node2.position, node3.position)
            .enclosingBoundingBox()
            .enclosingTilesRect(16)
        assertTrue(nodesRect.size <= 4) // fits in cache
        ...
        assertTrue(cache.getRelationsForNode(3L).containsExactlyInAnyOrder(listOf(rel1)))
        assertTrue(cache.getRelationsForWay(1L).containsExactlyInAnyOrder(listOf(rel1)))
    }
    ...
    @Test fun `update does add way to relationsByElementKey entry if entry already exists`() {
        ...
        assertTrue(
            cache.getRelationsForNode(3L) { relationDB.getAllForNode(it) }
                .containsExactlyInAnyOrder(listOf(rel2))
        )
    ...
    @Test fun `getMapDataWithGeometry fetches only part if something is already cached`() {
        ...
        val rel2 = rel(2, members = listOf(RelationMember(ElementType.WAY, 1L, "")))
        val elementDB = mock<ElementDao>()
        every { elementDB.getAll(node1rect.asBoundingBox(16)) } returns
            listOf(node1, node3, way2, way3, rel1)
        val cache = MapDataCache(16, 4, 10, { elementDB.getAll(it) to emptyList() }, { emptyList() })
...
private fun getEmptyMapDataCache() =
    MapDataCache(16, 4, 10, { emptyList<Element>() to emptyList() }, { emptyList() })

private fun MapDataCache.getNodes(ids: Collection<Long>) =
    getNodes(ids, fetch = { throw IllegalStateException() })

private fun MapDataCache.getElements(keys: Collection<ElementKey>) =
    getElements(keys, fetch = { throw IllegalStateException() })

private fun MapDataCache.getGeometries(keys: Collection<ElementKey>) =
    getGeometries(keys, fetch = { throw IllegalStateException() })

private fun MapDataCache.getWaysForNode(id: Long) =
    getWaysForNode(id, fetch = { throw IllegalStateException() })

private fun MapDataCache.getRelationsForNode(id: Long) =
    getRelationsForNode(id, fetch = { throw IllegalStateException() })

private fun MapDataCache.getRelationsForWay(id: Long) =
    getRelationsForWay(id, fetch = { throw IllegalStateException() })

private fun MapDataCache.getRelationsForRelation(id: Long) =
    getRelationsForRelation(id, fetch = { throw IllegalStateException() })
