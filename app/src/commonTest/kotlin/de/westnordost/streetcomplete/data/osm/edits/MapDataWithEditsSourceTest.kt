...
class MapDataWithEditsSourceTest {
    ...
    private lateinit var editsSource: ElementEditsSource
    ...
    private lateinit var mapDataListener: MapDataSource.Listener
        ...
        mapDataSource = mock() {
            every { get(any(), any()) } calls { (type: ElementType, id: Long) ->
                when (type) {
                    ...
                    WAY -> mapData.getWay(id)
                    RELATION -> mapData.getRelation(id)
                }
            }
            ...
            every { getRelation(any()) } calls { (id: Long) ->
                mapData.getRelation(id)
            }
            every { getNodes(any()) } calls { (ids: Collection<Long>) ->
                ids.mapNotNull { mapData.getNode(it) }
            }
            every { getWays(any()) } calls { (ids: Collection<Long>) ->
                ids.mapNotNull { mapData.getWay(it) }
            }
            every { getRelations(any()) } calls { (ids: Collection<Long>) ->
                ids.mapNotNull { mapData.getRelation(it) }
            }
            every { getAll(any()) } calls { (elementKeys: Collection<ElementKey>) ->
            ...
            every { getWaysForNode(any()) } calls { (id: Long) ->
                mapData.ways.filter { it.nodeIds.contains(id) }
            }
            every { getRelationsForNode(any()) } calls { (id: Long) ->
                mapData.relations.filter { r -> r.members.any { it.ref == id && it.type == NODE } }
            }
            every { getRelationsForWay(any()) } calls { (id: Long) ->
                mapData.relations.filter { r -> r.members.any { it.ref == id && it.type == WAY } }
            }
            every { getRelationsForRelation(any()) } calls { (id: Long) ->
                mapData.relations.filter { r -> r.members.any { it.ref == id && it.type == RELATION } }
            }
            every { getRelationsForElement(any(), any()) } calls { (type: ElementType, id: Long) ->
                mapData.relations.filter { r -> r.members.any { it.ref == id && it.type == type } }
            }
            every { getGeometry(any(), any()) } calls { (type: ElementType, id: Long) ->
                when (type) {
                    NODE -> mapData.getNodeGeometry(id)
                    WAY -> mapData.getWayGeometry(id)
                    RELATION -> mapData.getRelationGeometry(id)
                }
            }
            every { getGeometries(any()) } calls { (keys: Collection<ElementKey>) ->
                keys.mapNotNull { key ->
                    when (key.type) {
            ...
            every { getMapDataWithGeometry(any()) } calls { (bbox: BoundingBox) ->
            ...
            every { addListener(any()) } calls { (listener: MapDataSource.Listener) ->
    ...
    fun `get returns updated element updated from updated element`() {
        ...
        every { editsSource.getAllUnsynced() } returns listOf(
            edit(action = action2),
            edit(action = action3),
        )
    ...
    fun `conflict on applying edit is ignored`() {
        ...
        every { editsSource.getAllUnsynced() } returns listOf(edit(action = action))
    ...
    fun `onAddedEdit relays updated elements`() {
        ...
        verify { listener.onUpdated(updated = expectedMapData, deleted = listOf()) }
    ...
    fun `onAddedEdit relays deleted elements`() {
        ...
        verify {
            listener.onUpdated(
                updated = MutableMapDataWithGeometry(),
                deleted = listOf(ElementKey(NODE, 1))
            )
        }
    ...
    fun `onDeletedEdits relays updated element`() {
        ...
        verify {
            listener.onUpdated(
                updated = MutableMapDataWithGeometry(listOf(n), listOf(p)),
                deleted = listOf()
            )
        }
    }
    ...
    fun `onDeletedEdit relays elements if only their geometry is updated`() {
        ...
        verify { listener.onUpdated(updated = MutableMapDataWithGeometry(listOf(n, w, r), listOf(g, l, rg)), deleted = listOf()) }
    }
    ...
    fun `onUpdate passes through mapData because there are no edits`() {
        ...
        verify { listener.onUpdated(expectedMapDataWithGeometry, expectedDeletions) }
    }
    ...
    fun `does call onUpdated when not all deleted elements are already deleted`() {
        ...
        verify { listener.onUpdated(updatedMapData, expectedDeletions) }
    }
    ...
    fun `does call onUpdated when not all updated elements stayed the same`() {
        ...
        verify { listener.onUpdated(updatedMapData, emptyList()) }
    }
    ...
    fun `onReplacedForBBox passes through mapData because there are no edits`() {
        ...
        verify { listener.onReplacedForBBox(bbox, expectedMapDataWithGeometry) }
    }
    ...
    private fun create() = MapDataWithEditsSourceImpl(mapDataSource, editsSource, geometryCreator)
    ...
    private fun editsControllerNotifiesDeletedEdit(editedElementKeys: List<ElementKey>, createdElementKeys: List<ElementKey>) {
        every { editsSource.getIdProvider(any()) } returns ElementIdProvider(createdElementKeys)
        val action = mock<ElementEditAction>()
