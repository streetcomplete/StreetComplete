...
class OpeningHoursParserAndGeneratorTest {
    ...
    @Test fun `accepted valid and supported rule`() {
        // time points
        accept("Mo 09:00", allowTimePoints = true)
        ...
        accept("PH,Mo-Th 08:00-10:00", "Mo-Th,PH 08:00-10:00")
        accept("Mo-Th,PH 08:00-10:00")
        // weekday ranges
    ...
    private fun parseAndGenerate(oh: String, allowTimePoints: Boolean): String? =
        oh.toOpeningHoursOrNull(lenient = true)
            ?.toHierarchicOpeningHours(allowTimePoints)
            ?.toOpeningHours()?.toString()

    private fun parse(oh: String, allowTimePoints: Boolean): HierarchicOpeningHours? =
        oh.toOpeningHoursOrNull(lenient = true)
            ?.toHierarchicOpeningHours(allowTimePoints)

    private fun reject(oh: String, allowTimePoints: Boolean = false) {
        assertNull(parse(oh, allowTimePoints))
    }

    private fun accept(oh: String, result: String = oh, allowTimePoints: Boolean = false) {
        assertEquals(result, parseAndGenerate(oh, allowTimePoints))
    }
}
