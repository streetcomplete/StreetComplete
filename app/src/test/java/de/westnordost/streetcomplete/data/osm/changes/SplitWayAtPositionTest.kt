...
class SplitWayAtPositionTest {
    ...
    fun `do not allow negative delta`() {
        SplitWayAtPosition(way, node1, node2, -0.1)
    }

    @Test(expected = IllegalArgumentException::class)
    fun `do not allow delta above or equal 1`() {
        SplitWayAtPosition(way, node1, node2, 1.0)
    }

    @Test(expected = IllegalArgumentException::class)
    fun `do not allow delta 0 if split is at first node of way`() {
        SplitWayAtPosition(way, node1, node2, 0.0)
    }
    ...
    fun `ensure that second node is actually in the way`() {
        ...
        SplitWayAtPosition(way, node1, node3, 0.5)
    }
