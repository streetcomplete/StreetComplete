...
class OsmNotesDownloaderTest {
    ...
    private lateinit var downloader: OsmNotesDownloader
    ...
    @Before fun setUp() {
        ...
        downloader = OsmNotesDownloader(notesServer, noteDB, noteQuestDB, createNoteDB, preferences, OsmNoteQuestType(), avatarsDownloader)
        downloader.questListener = mock()
    }
    ...
    @Test fun `delete obsolete quests`() {
        ...
        doAnswer { invocation: InvocationOnMock ->
            val handler = invocation.arguments[1] as Handler<Note>
            handler.handle(note1)
        }.on(notesServer).getAll(any(), any(), any(), any())

        on(noteQuestDB.deleteAllIds(any())).thenReturn(1)

        downloader.download(BoundingBox(0.0, 0.0, 1.0, 1.0), null, 1000)

        verify(noteQuestDB).deleteAllIds(listOf(13L))
        verify(downloader.questListener!!).onQuestsRemoved(any(), any())
    }
