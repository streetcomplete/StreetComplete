...
internal fun parseGpx(input: String): List<LatLon> {
    // make sure sequences are exhausted in correct order
    return parseGpxFile(input.byteInputStream()).flatMap { it.toList() }.toList()
}
