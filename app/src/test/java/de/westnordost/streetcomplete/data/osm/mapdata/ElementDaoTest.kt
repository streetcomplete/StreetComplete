...
class ElementDaoTest {
    ...
    @Test fun getNode() {
        dao.get(ElementType.NODE, 1L)
        verify(nodeDao).get(1L)
    }

    @Test fun deleteNode() {
        dao.delete(ElementType.NODE, 1L)
    ...
    @Test fun getWay() {
        dao.get(ElementType.WAY, 1L)
        verify(wayDao).get(1L)
    }

    @Test fun deleteWay() {
        dao.delete(ElementType.WAY, 1L)
    ...
    @Test fun getRelation() {
        dao.get(ElementType.RELATION, 1L)
        verify(relationDao).get(1L)
    }

    @Test fun deleteRelation() {
        dao.delete(ElementType.RELATION, 1L)
    ...
    @Test fun deleteAllElements() {
        dao.deleteAll(listOf(
            ...
            ElementKey(ElementType.WAY,0),
            ElementKey(ElementType.RELATION,0)
        ))
