...
import de.westnordost.streetcomplete.data.osm.changes.HasElementTagChanges
import de.westnordost.streetcomplete.ktx.copy
import de.westnordost.streetcomplete.util.distanceTo

/** Uploads the tag changes made for one element.
...
class SingleOsmElementTagChangesUploader @Inject constructor(private val mapDataApi: MapDataApi) {
    ...
    fun upload(changesetId: Long, change: HasElementTagChanges, dbElement: Element): ElementUpdates {
        ...
        while(true) {
            ...
            try {
                ...
                element = handleConflict(change, element, e)
    ...
    private fun handleConflict(change: HasElementTagChanges, element: Element, e: OsmConflictException): Element {
        ...
        if (change.isApplicableTo(newElement) == false) {
