...
class OsmQuestUpdater @Inject constructor(
    ...
    private val listener = object : OsmElementSource.ElementUpdatesListener {
        override fun onUpdated(bbox: BoundingBox, mapDataWithGeometry: MapDataWithGeometry) {
            update(bbox, mapDataWithGeometry)
        }
    }

    init {
        osmElementSource.addQuestStatusListener(listener)
    }

    fun update(bbox: BoundingBox, mapData: MapDataWithGeometry) {
            ...
            for (questType in questTypes) {
                launch(Dispatchers.Default) {
                    ...
                    if (!countryBoundaries.intersects(bbox, questType.enabledInCountries)) {
                        ...
                        Log.d(TAG, "$questTypeName: Found $i quests in ${questSeconds}ms")
                    }
                }
            }
        }
