...
class AddSportForm : ImageListQuestAnswerFragment() {
    ...
    override fun getItems(): Array<Item> {
        val sportsList = ALL_SPORTS_VALUES.sortedBy(countryInfo.popularSports)

        // only first 24 items (6 rows)
        val arraySize = Math.min(sportsList.size, 24)
        return sportsList.subList(0, arraySize).toTypedArray()
    }
    ...
    companion object {
        private val ALL_SPORTS_VALUES = arrayOf(
