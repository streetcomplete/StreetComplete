...
fun questTypeRegistry(
    ...
    */
    ...
    11 to AddAmenityCover { featureDictionaryFuture.get().getFeature(it) },
    ...
    49 to CheckExistence { featureDictionaryFuture.get().getFeature(it) },
    ...
    79 to CheckShopType(),
    80 to AddPlaceName { featureDictionaryFuture.get().getFeature(it) },
    77 to CheckOpeningHoursSigned { featureDictionaryFuture.get().getFeature(it) },
    81 to AddOpeningHours { featureDictionaryFuture.get().getFeature(it) },
    ...
    156 to CheckShopExistence { featureDictionaryFuture.get().getFeature(it) }, // after opening hours and similar so they will be preferred if enabled
