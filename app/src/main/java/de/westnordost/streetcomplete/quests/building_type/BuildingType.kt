...
enum class BuildingType(val osmTag: String?, val subTypes: List<BuildingType>? = null) {
    ...
    RESIDENTIAL   ("building=residential", listOf(DETACHED, APARTMENTS, SEMI_DETACHED, TERRACE, HOUSE, FARM, HUT, BUNGALOW, HOUSEBOAT, STATIC_CARAVAN, DORMITORY)),
    COMMERCIAL    ("building=commercial", listOf(OFFICE, INDUSTRIAL, RETAIL, WAREHOUSE, KIOSK, HOTEL, STORAGE_TANK)),
    CIVIC         ("building=civic", listOf(SCHOOL, UNIVERSITY, HOSPITAL, KINDERGARTEN, SPORTS_CENTRE, TRAIN_STATION, TRANSPORTATION, COLLEGE, GOVERNMENT, STADIUM)),
    RELIGIOUS     ("building=religious", listOf(CHURCH, CATHEDRAL, CHAPEL, MOSQUE, TEMPLE, PAGODA, SYNAGOGUE, SHRINE)),
    ...
    companion object {
        fun getByTag(key: String, value: String): BuildingType? {
            return values().find { it.osmTag == "$key=$value" }
        }
    }
}
