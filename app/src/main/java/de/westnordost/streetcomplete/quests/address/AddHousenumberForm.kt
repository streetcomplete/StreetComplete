...
private val VALID_CONSCRIPTIONNUMBER_REGEX = Regex("\\p{N}{1,6}")

// i.e. 99999/a, 9/a, 99/9, 99a, 99 a, 9 / a
internal val VALID_HOUSENUMBER_REGEX = Regex("\\p{N}{1,5}((\\s?/\\s?\\p{N})|(\\s?/?\\s?\\p{L}))?")

// i.e. 9, 99, 999, 999, 9A, 9 A
internal val VALID_BLOCKNUMBER_REGEX = Regex("\\p{N}{1,4}(\\s?\\p{L})?")

private fun HousenumberAnswer.looksInvalid(additionalValidHousenumberRegex: String?): Boolean {
