...
class OsmoseDao(
    ...
    private val ignoredItems by lazy { prefs.getString(questPrefix(prefs) + PREF_OSMOSE_ITEMS, "")!!
        .split(',').map { it.trim() }.toMutableSet() }

    fun reloadIgnoredItems() {
        ignoredItems.clear()
        ignoredItems.addAll(
            prefs.getString(questPrefix(prefs) + PREF_OSMOSE_ITEMS, "")!!
                .split(',').map { it.trim() }.toMutableSet()
        )
    }

    fun download(bbox: BoundingBox) {
        if (!prefs.getBoolean(questPrefix(prefs) + PREF_OSMOSE_ENABLE_DOWNLOAD, false)) return
    ...
    fun getAll(): Map<ElementKey, OsmoseIssue> {
        ...
        }.toMap().filterValues { it.item !in ignoredItems }
    ...
    fun get(element: ElementKey): OsmoseIssue? {
        return db.queryOne(NAME,
            ...
            .takeIf { it?.item !in ignoredItems }
    }
