...
class AddPowerPolesMaterial : OsmFilterQuestType<PowerPolesMaterial>() {
    ...
    override fun getHighlightedElements(element: Element, getMapData: () -> MapDataWithGeometry) =
        getMapData().filter("nodes with power = pole or man_made = utility_pole") +
        // and also show the (power) lines themselves
        getMapData().filter("ways with power ~ line|minor_line or communication = line or telecom = line")
