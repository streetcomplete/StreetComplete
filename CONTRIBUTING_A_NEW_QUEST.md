...
Each quest is associated with a number in this list. These numbers are used to identify the quest uniquely and can be used to save presets as QR codes. When adding a new quest, use a new number that is not yet in the list. Put the quest in order of priority, even if it means the numbers are not sorted.
